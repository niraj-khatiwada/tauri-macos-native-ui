...
class NativeMenuManager: NSObject, NSMenuDelegate {
    ...
    func openMenu(x: Double, y: Double, itemsJson: String, focusParentWindow: Bool) {
        ...
        if targetWindow == nil {
            ...
            targetWindow =
                windows.first(where: {
                    NSMouseInRect(mouseLocation, $0.frame, false) && $0.isVisible
                })
        ...
        let isPopoverWindow = String(describing: type(of: window)).contains("Popover")

        if focusParentWindow && !isPopoverWindow {
    ...
    private func buildMenuItem(from data: MenuItemData) -> NSMenuItem {
        ...
        menuItem.state = (data.checked ?? false) ? .on : .off
